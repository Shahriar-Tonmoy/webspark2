...
export default function Nav(props: {setlogo: (data: string) => void; logostr: string; pagestate: SECTIONS; setpagestate: (data: SECTIONS) => void}) {
    return (
        <nav className="bg-white shadow-lg fixed z-20 w-screen">
            <div className="relative group">
        {props.pagestate !== SECTIONS.NAV && <div className="hidden bg-slate-200 absolute z-10 top-1/2 px-8 py-2 left-1/2 group-hover:flex text-4xl rounded-xl text-[#4C35DE] justify-center cursor-pointer shadow-xl" onClick={() => {props.setpagestate(SECTIONS.NAV)}}><p className="my-auto text-lg mx-2"><TfiPencilAlt /></p> <p className="my-auto">Edit</p></div>}
        </div>
